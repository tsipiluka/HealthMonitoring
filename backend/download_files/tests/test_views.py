...
class TestFileDownload(APITestCase):
    def test_download_file(self):
        ...
        response = client.get(
            reverse("download-file", kwargs={"medical_finding_id": medical_finding})
        )
    ...
    def test_download_invalid_medical_finding_id(self):
        ...
        client.credentials(HTTP_AUTHORIZATION=f"Bearer {refresh.access_token}")
        response = client.get(
            reverse("download-file", kwargs={"medical_finding_id": "invalid"})
        )
    ...
    def test_download_valid_random_medical_finding_id(self):
        ...
        response = client.get(
            reverse(
                "download-file",
                kwargs={"medical_finding_id": "00000000-0000-0000-0000-000000000000"},
            )
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
